...
def create_attendance_table(doc, monday_date, employees, title_text="每周考勤表"):
    """创建单周考勤表 - 完全参照原文件格式"""
    # 从周一开始生成一周7天的日期
...
def generate_attendance_sheets(mondays, employees, title_text="每周考勤表"):
    """生成连续的每周考勤表"""
    doc = Document()
    ...
    for i, monday in enumerate(mondays):
        create_attendance_table(doc, monday, employees, title_text)
        if i < len(mondays) - 1:
            doc.add_page_break()
...
class AttendanceApp:
    ...
    def generate(self):
        ...
        try:
            ...
            mondays = get_week_range_from_dates(start, end)

            title_text = self.title_var.get().strip()
            if not title_text:
                title_text = "每周考勤表"

            doc = generate_attendance_sheets(mondays, self.employees, title_text)

            # 用起止日期命名文件
            filename = f"{title_text}_{start_str}_至_{end_str}.docx"
